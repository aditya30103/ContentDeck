...
| 9 | P0 | UI | The mobile viewport saga (5 PRs, fully documented in screenshots + diagnostics). Facts established: fresh iOS standalone installs get a status-bar-inset webview (innerHeight 793 vs screen 852); the 59pt bottom strip is OS-owned and unclaimable (identical day one vs now); env() reports a bogus 59px top inset; 100dvh is untrustworthy. Final resolution per user benchmark (mobile-light.jpeg): compact 44px unpadded bar restored, HomePage top offset restored, height:100% chain + static chrome + scrollable sheets retained (PRs #60–#66). | ✅ CLOSED — device-confirmed; temp diagnostics removed (PR #67) |
