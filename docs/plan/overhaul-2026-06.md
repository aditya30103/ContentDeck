...
| 9 | P0 | UI | Bottom nav floats above a ~59pt dead band on device. SOLVED via diagnostics: the installed PWA runs in status-bar-inset mode (innerHeight 793 vs screen 852) — Aditya's install predates the v3.8 `black-translucent` metas and iOS snapshots config at install time. The dead strip is OS territory; iOS still reports env() insets there (hence #60's double-count). Fix: `--safe-*` vars gated on `html.vp-fullbleed` (innerHeight === screen.height), all consumers wired (PR #63). | 🟡 Shipped — Aditya must remove + re-add the home-screen icon, then confirm `fullbleed: true` + edge-to-edge in diagnostics; then remove temp diagnostics |
