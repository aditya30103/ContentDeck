...
| 9 | P0 | UI | Mobile working area wasted by stacked viewport lies (user-reported). Final root causes via device diagnostics: fresh iOS standalone installs get a status-bar-inset webview (innerHeight 793 vs screen 852) yet report a bogus env() top inset of 59px, AND 100dvh computes 59px short of the real viewport. Fix: height:100% chain (html/body/#root/column), no top padding, keep bottom env() pad (PRs #60–#65). | 🟡 Deployed (PR #65) — awaiting final device confirmation, then remove temp diagnostics |
