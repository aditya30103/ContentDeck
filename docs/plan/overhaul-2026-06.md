...
| 9 | P0 | UI | Bottom nav floats above a ~59pt dead band on device. SOLVED via diagnostics: the installed PWA runs in status-bar-inset mode (innerHeight 793 vs screen 852) — Aditya's install predates the v3.8 `black-translucent` metas and iOS snapshots config at install time. The dead strip is OS territory; iOS still reports env() insets there (hence #60's double-count). Fix evolution: #63 gated insets on a JS check that misfired post-reinstall (status bar over header); #64 applies env() insets unconditionally — correct for fresh full-bleed installs. | 🟡 Deployed (PR #64) — Aditya relaunches app, confirms header below status bar + diagnostics numbers; then remove temp diagnostics |
